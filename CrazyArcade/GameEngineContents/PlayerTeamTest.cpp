...
void PlayerTeamTest::LevelChangeStart(GameEngineLevel* _PrevLevel)
{

	{
		...
		//MapBackGround_->GetRenderer()->SetImage("Boss_ColMap.bmp");//Actor에 이미지 세팅해주고
	...
	MapBackGround_ = CreateActor<MapBackGround>((int)ORDER::BACKGROUND);//Actor 만들고
	MapBackGround_->GetRenderer()->SetImage("PlayUI.bmp");//Actor에 이미지 세팅해주고d
	MapBackGround_->GetRenderer()->SetPivot({ 400,300 });//윈도우기준 그려줄 위치 정해주고


	MapFrontBackGround_ = CreateActor<MapFront>((int)ORDER::FRONTBACKGROUND);//Actor 만들고
	MapFrontBackGround_->GetRenderer()->SetImage("BossStage_Back.bmp");//Actor에 이미지 세팅해주고
